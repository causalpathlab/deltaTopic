...
from nn.base_components import TotalMultiMaskedEncoder, TotalDeltaETMDecoder, BeyesianETMDecoder, BeyesianETMDecoder_rho, TotalSingleMaskedEncoder
...
class BETM_module(BaseModuleClass):
    """
    This is the test idea to fix the mode collapsing problem.
    Parameters
    ----------
    dim_input
        number of input genes for each dataset.
    total_genes
        Total number of different genes
    n_latent
        dimension of latent space
    ...
    def __init__(
        ...
        self.z_encoder = TotalSingleMaskedEncoder(
            n_input=dim_input,
            n_output=self.n_latent,
            mask = self.mask,
            n_hidden=dim_hidden_encoder,
        ...
        # TODO: use self.total_genes is dangerous, if we have dfferent sets of genes in spliced and un unspliced
        self.decoder = BeyesianETMDecoder_rho(self.n_latent , 
                                          self.total_genes, 
                                          pip0 = self.pip0_rho,
                                        )
    ...
    def loss(
        ...
        return LossRecorder(loss, reconstruction_loss, kl_local,
                            ...
                            reconstruction_loss_unspliced=reconstruction_loss, 
                            kl_beta = kl_divergence_beta, 
                            kl_rho = rho_kl, 
                            kl_delta = rho_kl) # here just for the ease of using trainingplan
...
class BayesianETM_module(BaseModuleClass):
    ...
    def __init__(
        ...
        # TODO: use self.total_genes is dangerous, if we have dfferent sets of genes in spliced and un unspliced
        self.decoder = BeyesianETMDecoder(self.n_latent , 
                                          self.total_genes, 
