...
import wandb
wandb.login()
wandb.init(entity="thisisyichen", project="TotalDeltaETM_decoupled")
# Input parser
...
parser.add_argument('--combine_method', type=str, help='how to combine to Zs', default='add')
parser.add_argument('--train_size', type=float, help='training size', default=1)
args = parser.parse_args()
# pass args to wand.config
wandb.config.update(args)
#%%
savefile_name = f"models/TotalDeltaETM_decoupled_allgenes_ep{args.EPOCHS}_nlv{args.nLV}_bs{args.bs}_combineby{args.combine_method}_lr{args.lr}_train_size{args.train_size}"
print(savefile_name)
...
#%%
# create model
from DeltaETM_model import TotalDeltaETM_decoupled
model = TotalDeltaETM_decoupled(adata_spliced, n_latent = args.nLV, combine_latent= args.combine_method)
#%%
# this has to be passed, otherwise pytroch lighting logging won't be passed to wandb
from pytorch_lightning.loggers import WandbLogger
import torch
wandb_logger = WandbLogger(project = 'TotalDeltaETM_decoupled')
model_kwargs = {"lr": args.lr, 'use_gpu':args.use_gpu, 'train_size':args.train_size}
#model_kwargs = {"lr": args.lr, 'train_size':args.train_size}
print(args)
